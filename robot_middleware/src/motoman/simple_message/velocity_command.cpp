...
#include "robot_middleware/motoman/simple_message/velocity_command.h"
...
bool VelocityCommand::load(ByteArray* buffer)
{
  ...
  if (!buffer->load(this->group_no_))
  {
    LOG_ERROR("Failed to load velocity command group_no");
    return false;
  }
...
bool VelocityCommand::unload(ByteArray* buffer)
{
  ...
  if (!buffer->unload(this->group_no_))
  {
    LOG_ERROR("Failed to unload velocity command group_no");
    return false;
  }
